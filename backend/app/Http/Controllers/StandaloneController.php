...
class StandaloneController extends Controller
{
    ...
    public function idVerification(Request $request)
    {
        ...
        try {
            $result = $this->idRunner->run($frontPath, $backPath);
        } finally {
            @unlink($frontPath);
            if ($backPath) {
                @unlink($backPath);
    ...
    public function liveness(Request $request)
    {
        ...
        return $this->respond($request, 'liveness', $this->livenessRunner->run($bytes));
    }
    ...
    public function faceMatch(Request $request)
    {
        ...
        return $this->respond($request, 'face_match', $this->faceMatchRunner->run($idBytes, $selfieBytes));
    }
    ...
    public function ageVerification(Request $request)
    {
        ...
        return $this->respond($request, 'age', $this->ageRunner->run($validated['dob'], $validated['bands'] ?? []));
    }
    ...
    public function credentialVerification(Request $request)
    {
        ...
        return $this->respond($request, 'credential', $this->credentialRunner->run($input));
    }
    ...
    public function kycCredential(Request $request)
    {
        ...
        try {
            $idResult = $this->idRunner->run($frontPath, $backPath);
        } finally {
            @unlink($frontPath);
            if ($backPath) {
                @unlink($backPath);
        ...
        $session = $this->sessions->recordCheck($session, $liveness = $this->livenessRunner->run($selfie));
        $checks[] = $liveness->toArray();
        $session = $this->sessions->recordCheck($session, $face = $this->faceMatchRunner->run($front, $selfie));
        $checks[] = $face->toArray();
        ...
        if (!$session->isTerminal()) {
            ...
            $session = $this->sessions->recordCheck($session, $credential = $this->credentialRunner->run($license));
